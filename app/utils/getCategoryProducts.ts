...
export const getCategoryProducts = (data: Product[] | undefined, category: Styles): Product[] => {
  ...
  if (data) {
    if (['Solitaire', 'Halo', 'Cluster', 'Three Stone'].includes(category)) {
      ...
      products = data.filter((product) =>
        product.attributes.pa_shoulders?.includes('Diamond') && product.category === 'WEDDING',
      )
    }
    else if (category === 'Shaped') {
      products = data.filter(
        (product) => product.attributes.pa_shaped?.length && ['WEDDING', 'ETERNITY'].includes(product.category)
      )
    }
